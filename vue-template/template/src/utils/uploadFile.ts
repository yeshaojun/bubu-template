...
interface UploadType {
  ...
  limit?: number
  errorCount?: number
  checkFn: (ext: string, hash: string) => Promise<{ uploaded: boolean; uploadedList: string[] }>
  uploadFn: (
    form: FormData,
...
export async function useUploadSliceFile({
  ...
  // 3.检查hash上传情况
  const { uploaded, uploadedList } = await checkFn(getFileExt(file.name), hash.value)
  if (uploaded) {
    // 已经上传成功
    success.value = true
    return
      ...
      const workLoop = async (deadline: IdleDeadline) => {
        ...
        while (count < chunks.value.length && deadline.timeRemaining() > 1) {
          ...
          if (count < chunks.value.length) {
            hashProgress.value = Number(((100 * count) / chunks.value.length).toFixed(2))
          } else {
